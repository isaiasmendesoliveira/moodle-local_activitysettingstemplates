...
- Uso conjunto de cor, ícone e texto para comunicar status, sem depender apenas da cor.
...
A seção do plugin está disponível nos formulários dos módulos de atividade/recurso instalados (`mod_*`). Para os módulos principais do Moodle, as configurações são selecionadas a partir de uma lista curada e centrada no professor, evitando campos internos, calculados ou relacionais.
...
Plugins de atividade de terceiros podem ser reconhecidos por meio de um fallback conservador, sem expor automaticamente os nomes brutos de campos do banco de dados.
...
Se uma configuração não existir, estiver bloqueada ou for incompatível com o formulário de destino, o plugin não a impõe.
